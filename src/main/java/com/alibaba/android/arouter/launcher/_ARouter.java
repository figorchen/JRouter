                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    ...
                    public void run() {
                        if (requestCode > 0) {  // Need start for result
                            if(postcard.getFragment() != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                                postcard.getFragment().startActivityForResult(intent, requestCode, postcard.getOptionsBundle());
                            } else if(postcard.getSupportFragment() != null) {
                                postcard.getSupportFragment().startActivityForResult(intent, requestCode, postcard.getOptionsBundle());
                            } else {
